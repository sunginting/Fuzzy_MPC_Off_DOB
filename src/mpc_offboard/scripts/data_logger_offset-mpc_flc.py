...
class DataLoggerLOSGuidance(object):
    def __init__(self):
        ...
        self.weights_MPC = rospy.Subscriber('/mpc/weights', Float32MultiArray, self.flc_weights_callback, queue_size=10)

        # AVG ERROR
        self.error_avg_sub = rospy.Subscriber('/flc/error_avg', Float32MultiArray, self.avg_error_callback, queue_size=10)

        # Estimate callback
        self.est_sub = rospy.Subscriber('/mpc/dist_estimate', Float32MultiArray, self.est_callback,queue_size=10)

        # Disturbance gain callback
    ...
    def flc_weights_callback(self, msg: Float32MultiArray):
        """
