...
class LOSGuidanceTrajectoryPublisher(object):
    ...
    def __init__(self):
        ...
        self.lookahead_distance = rospy.get_param("~lookahead_distance", 1.5)  # meter
        ...
        # Kp = 1/lookahead_distance. kappa kecil -> aksi integral lembut
        self.ilos_kappa = rospy.get_param("~ilos_kappa", 0.8)
        ...
        # ===================== VERTICAL LOS =====================
        # Aktifkan LOS vertikal (True) atau pakai proportional altitude lama (False)
        self.use_vertical_los = rospy.get_param("~use_vertical_los", True)
        # Lookahead distance vertikal (Delta_v), analog dengan delta horizontal
        self.lookahead_distance_v = rospy.get_param("~lookahead_distance_v", 2.0)  # meter
        self.cross_track_v_integral = 0.0
        self.cross_track_v_integral_max = rospy.get_param("~cross_track_integral_max", 2.0)
        self.ilos_v_kappa = rospy.get_param("~ilos_v_kappa", 0.7)
    ...
    def compute_los_guidance(self, pos_ned, dt):
        ...
        chi_d = gamma_p - np.arctan2(((Kp*ye)+(Ki*yint)), delta)
        ...
        # ============================================================
        # 6. Vertical LOS guidance (Lekkas & Fossen, Sec. 4.3.2, eq. 45/46/51)
        # ============================================================
        seg_horiz_len = np.linalg.norm(seg_vec)  # panjang horizontal segmen aktif (N,E)

        if self.use_vertical_los and seg_horiz_len > 1e-3:
            d0 = self.positions[closest_seg_idx][2]      # D awal segmen
            d1 = self.positions[closest_seg_idx + 1][2]  # D akhir segmen
            dd = d1 - d0                                  # perubahan Down sepanjang segmen

            # Vertical path-tangential angle (eq. 46):
            #   alpha_p = atan2(-(z_{k+1}-z_k), x_{k+1}-x_k), x = arah horizontal segmen.
            #   alpha_p > 0 berarti path menanjak (D mengecil).
            alpha_p = np.arctan2(-dd, seg_horiz_len)

            # Koordinat lokal di bidang vertikal terhadap awal segmen:
            #   (x - x_k) = jarak along-horizontal = tau * panjang horizontal
            #   (z - z_k) = selisih altitude (D)
            s_horiz = closest_tau * seg_horiz_len
            dz = pos_z - d0

            delta_v = self.lookahead_distance_v
            # Vertical cross-track error (eq. 45): ze = (x-xk) sin(ap) + (z-zk) cos(ap)
            ze = s_horiz * np.sin(alpha_p) + dz * np.cos(alpha_p)
            ze_int = self.cross_track_v_integral
            ze_int_dot = (ze*delta_v)/(delta_v**2 + (ze + (self.ilos_v_kappa*ze_int))**2)
            ze_int += dt * ze_int_dot
            ze_int = float(np.clip(ze_int,-self.cross_track_v_integral_max, self.cross_track_v_integral_max))
            self.cross_track_v_integral = ze_int

            Kp_v = 1/delta_v
            Ki_v = Kp_v*self.ilos_v_kappa

            theta_d = alpha_p - np.arctan((Kp_v*ze)+(Ki_v*ze_int))

            vel_z = self.ref_speed * np.tan(theta_d)
            vel_z = float(np.clip(vel_z, -self.max_vertical_speed, self.max_vertical_speed))
            self.vertical_cross_track = ze
        else:
            # Segmen (hampir) vertikal atau vertical LOS dimatikan -> proportional altitude lama
            altitude_error = los_point_ned[2] - pos_z
            vel_z = self.altitude_gain * altitude_error
            vel_z = float(np.clip(vel_z, -self.max_vertical_speed, self.max_vertical_speed))
            self.vertical_cross_track = altitude_error
